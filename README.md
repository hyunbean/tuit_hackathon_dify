...
| 담당 | 산출물 |
|---|---|
| Dify 에이전트 워크플로우 3종 설계·구축 | `*.yml` DSL export (아래 §워크플로우 구성) |
| 품질 검증 하네스 | [`eval/`](eval/) — 골든셋 60케이스 + 결정론적 채점 + CI 회귀 평가 |
| 레퍼런스 백엔드 + 웹 데모 | [`server/`](server/) — 협업 설계 문서의 API 계약을 실행 가능한 코드로 구현 |
| 지식베이스 설계 | [`kb/`](kb/) — 다국어 별칭·카피·안전 문구 가이드 3종 |
...
> 2026-07-22 이후 재현 평가한 결과**이며, 해커톤 진행 당시의 실전 기록이 아닙니다.
> `gpt-4o-mini` (temperature 0.1) + Cohere `rerank-multilingual-v3.0` 기준.

| 측정 | 결과 |
|---|---|
| 1차 측정 (골든셋 60케이스) | 57/60 · 95% |
| 실패 3건 진단·패치 후 재측정 | **60/60 · 100%** — 단, **튜닝에 쓴 골든셋으로 잰 수치** |
| 별도 holdout 재측정 (15건, `[사후측정 2026-07-28]`) | **14/15 · 93%** ← 이 저장소가 뒷받침할 수 있는 보수적 수치 |

- **규칙 기반 사전 필터 효과** — 확신도 미달 제보는 LLM을 호출하지 않고 분기. 해당 경로 평균 응답 **1.14s vs 정상 경로 4.19s (약 73% 단축)**, 그 비율만큼 API 비용도 미발생.
- **꼬리 지연** — Product Normalizer는 평균 4.62s이지만 **p95 12.29s(평균의 2.7배)**. 실무 SLO는 평균이 아니라 p95/p99로 정해야 한다는 걸 이 수치가 보여줍니다.

📄 **1차 측정 원표 · 실패 3건의 가설 기각 과정(검색 슬롯 경쟁 → 프롬프트 패치 → 우즈베크어 동형이의어 발견) · 지연 분포 전체 · holdout 설계 근거:**
[`docs/eval-2026-07-22.md`](docs/eval-2026-07-22.md) · 실패 케이스 포스트모템 [`docs/postmortems/2026-07-22-normalizer-3-uzbek-failures.md`](docs/postmortems/2026-07-22-normalizer-3-uzbek-failures.md)
...
- **"60/60 100%"는 튜닝에 쓴 골든셋으로 잰 수치** — 별도 holdout 재측정은 14/15(93%)로 더 보수적.
- **N40(단위 매핑)이 패치 후에도 불안정** — 4회 중 3회 실패·1회 성공. 상품코드와 단위 확신도를 단일 `matchConfidence`에 합쳐 판단하는 설계가 원인.
- **오타 일반화 부족** — holdout에서 새로 발견된 `pomidorr` 오타가 `UNKNOWN`으로 실패. 기존 패치는 특정 오타 3건에만 국소 적용됐음.
- **Product Normalizer의 꼬리 지연이 평균의 2.7배** (p95 12.29s) — 원인 미진단.
- **Price Insight/Report Inspector의 p95/p99는 n=10이라 신뢰 불가** — 상위 1~2개 관측값에 불과.
- **해커톤 당시 실행 로그 부재** — 본 문서의 실측치는 전부 사후 재현 결과.
- **LLM 출력 검증이 수동 JSON 파싱에 의존** — Dify의 structured output(JSON Schema) 미사용.
- **금칙어 필터·fallback 메시지가 우즈베크어 미지원** (현재 영/한/러, fallback은 영/한).
- **골든셋 크기·정확도 기준선에 CI gate 없음** — 회귀 평가는 돌지만 임계치 미달 시 실패시키는 게이트는 미구현.

📄 **각 항목의 원인과 해결 방법 전문:** [`docs/limitations.md`](docs/limitations.md)
