...
Knowledge Retrieval 노드가 공유했습니다. import 후 아래 내용을 담은 지식베이스를
생성해 각 Knowledge Retrieval 노드에 연결해야 합니다.

| 담당 영역 | 내용 |
|---|---|
| 상품 별칭 가이드 | 상품별 다국어 별칭(uz/ru/en), 변형(variant), 단위 표현 매핑 |
| 가격 설명 카피 가이드 | 판정별 설명 문구, 데이터 출처 표기 원칙 |
| 안전 문구 가이드 | 중립 표현 가이드, 판매자 비난 금지 원칙, 운영자 체크리스트 템플릿 |
